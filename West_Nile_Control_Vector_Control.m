function [tau,ul,ua,X,J,J_comp,final_treatment_time,X0,T] = West_Nile_Control_Vector_Control(N,K,Tf,larvicide_type,ul0,ua0)
...
%J=X(13,N)+c_e*(X(1,N)+X(2,N))+cl*sum(ul.^2)+ca*sum(ua.^2)+cT*sum(tau.^2);
...
% sum_{i=1}^N{mu_t^+(i)*(tau_i-tau_max)+mu_t^-(i)*(t(i)-tau_i)-mu_a^-(i)*ua-mu_l^-(i)*ul+mu_a^+(u_a(i)-1)+mu_l^+(u_l(i)-1)}=0
...
C = p(11);             %mosquito larva carrying capacity
...
ic = [ic_E;0;C;0;ic_V;0;0;Nh;0;0;0;0;0;0];
